...
function VerifyProfileEmail({ isOpen, closeModal, reFetch, id }: any) {
  ...
  const onSubmit = async (data: FormSchema) => {
    const res = await Post({
      url: `/agents/verifyprofileemail/${id}/${data.token}`,
      body: {},
      method: 'PUT',
    });
    if (!res.ok) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='space-y-2'>
          <div className='grid gap-6 sm:grid-cols-2'>
            ...
            <FormInput
              ...
              form={form}
              type='text'
              placeHolder='please use the token sent to agent email'
              autoComplete='off'
              className='col-span-full'
              required
            />
